  ...
  try {
    ...
    if (process.platform === 'win32') {
      ...
      try {
        ...
        try {
          // Alternative PowerShell command for disk detection
          const psInfo = execSync('powershell "Get-PhysicalDisk | Select-Object MediaType, BusType | ConvertTo-Json"', { encoding: 'utf-8' });
          systemCommandOutput = psInfo;
          ...
          try {
            ...
            if (ssdDisks.length > 0) {
              diskType = 'SSD';
            } else if (hddDisks.length > 0) {
              diskType = 'HDD';
            } else {
              diskType = 'SSD'; // Default fallback
            }
          } catch (parseError) {
          ...
          diskType = 'SSD';
          systemCommandOutput = `wmic failed: ${e.message}, powershell failed: ${psError.message}`;
        }
        fsType = 'NTFS';
      }
    } else {
      ...
      try {
        ...
        // Get filesystem type using stat command (more reliable cross-platform)
        try {
          const statInfo = execSync('stat -f -c %T . 2>/dev/null || stat -f . 2>/dev/null || df -T . | tail -1 | awk "{print \\$2}"', { encoding: 'utf-8' }).trim();
          if (statInfo && !statInfo.includes('not supported')) {
            fsType = statInfo;
          } else {
            // Fallback to df command
            const dfInfo = execSync('df -T . | tail -1 | awk "{print \\$2}" 2>/dev/null || echo "unknown"', { encoding: 'utf-8' }).trim();
            fsType = dfInfo !== 'unknown' ? dfInfo : (process.platform === 'darwin' ? 'APFS' : 'ext4');
          }
        } catch (statError) {
...
async function runRealBenchmark(label: string, size: number): Promise<DatasetEvidence> {
  ...
  // Pre-clean database for this project and any existing project with same rootPath
  ...
  await db.project.deleteMany({ where: { rootPath } }).catch(() => {});
  ...
  // Final resume validation - always required to ensure complete state
  console.log(`[${label}] Performing final resume validation...`);
...
async function captureDatabaseChecksum(projectId: string): Promise<string> {
  ...
  // Create a more comprehensive checksum that includes chunk content hashes
  const dataToHash = analyses.map(a => ({
    ...
    chunks: a.chunks.map(c => ({
      ...
      contentHash: crypto.createHash('sha256').update(c.content).digest('hex'), // Hash the content instead of storing full content
